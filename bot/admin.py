...
class ActivityAdmin(admin.ModelAdmin):
    ...
    def export_button(self, obj):
        ...
        if obj.is_active:
            ...
            return mark_safe(
                ...
                f'title="При экспорте будут удалены все сообщения об активности у всех пользователей (независимо от статуса участия)">'
    ...
    def export_to_google_sheets(self, request, activity_id):
        ...
        try:
            ...
            if result:
                messages.success(request, f'Данные активности "{activity.name}" успешно экспортированы в Google Sheets. Лист: Лист1. Сообщения об активности удалены у всех пользователей (независимо от статуса участия).')
...
class ActivityHistoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Техническая информация', {
            ...
            'classes': ('collapse',),
            'description': 'При установке галочки "Экспортировано в Google Sheets" автоматически удаляются все сообщения об этой активности у всех пользователей (независимо от статуса участия). При любом редактировании данных происходит автообновление в Google Sheets (Лист1). При снятии галочки можно редактировать данные, а при повторной установке - обновить в таблице. Существующие записи с одинаковыми пользователем, событием и временем будут заменены новыми данными.'
        }),
    )
    ...
    def export_to_google_sheets(self, request, activity_history_id):
        ...
        try:
            ...
            if result:
                ...
                messages.success(request, f'Данные успешно экспортированы в Google Sheets. Лист: Лист1. Сообщения об активности удалены у всех пользователей (независимо от статуса участия).')
